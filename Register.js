...
import { register1, emailVarification, clearErrors } from '../../actions/userActions'
...
const Register = () => {
    ...
    }, [dispatch, navigate, alert, isAuthenticated, error, data])

    const codeHandler = (data, e) => {
        ...
        if (code === codeConfirm.data) {
            dispatch(register1(formData))
            reset();
        } else {
            window.alert("Invlid Security Code");
            setCode('');
            return false;
        }
    }
